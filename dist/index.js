...
var uploadArtifact = (repoName, revision) => __async(void 0, null, function* () {
  ...
  if (mainBranch && mainBranch !== branch) {
    buildArtifactName = artifactPostfix ? `${repoName}-${revision}"-"${artifactPostfix}` : `${repoName}-${revision}`;
  }
  const buildArtifactFilename = `${buildArtifactName}.${packageExtension}`;
  yield (0, import_exec3.exec)(`cp ./build.${packageExtension} ./${buildArtifactFilename}`);
