...
var createGitRevision = () => __async(void 0, null, function* () {
  ...
  const gitSHA5 = import_github.context.sha ? import_github.context.sha.slice(-5) : "";
  let revision = "";
  if (versionType !== VersionType.datehash) {
    ...
    revision = new Date().toISOString().replace(/-/g, "").replace(/:/g, "").replace(/\./g, "");
    revision = revision + gitSHA5;
  }
  const releaseMessage = (yield (0, import_exec.getExecOutput)(`git log -n 1 --pretty=format:%B`)).stdout;
...
var generateCurlCredential = (props) => {
  const { artifactToken: artifactToken2, artifactUsername: artifactUsername2, artifactPassword: artifactPassword2 } = props;
  if (artifactToken2) {
...
var generateArtifactUrl = (props) => {
  const { artifactHost: artifactHost2, artifactPath: artifactPath2, revision, buildArtifactFilename } = props;
...
var deploy = (props) => __async(void 0, null, function* () {
...
var createBuildInfo = (props) => __async(void 0, null, function* () {
  ...
  const { revision, artifacts, artifactPath: artifactPath2, startTime } = props;
  ...
  (0, import_fs.writeFileSync)((0, import_path.join)("build-info.json"), JSON.stringify(buildInfo), {
    encoding: "utf8"
  });
  return id;
});
var uploadBuildInfo = (credentials, artifactHost2) => __async(void 0, null, function* () {
  const credentialStr = generateCurlCredential(__spreadValues({}, credentials));
  yield (0, import_exec2.exec)(
    `curl -X PUT ${credentialStr} ${artifactHost2}/artifactory-build-info -T build-info.json`
  );
});
...
var uploadArtifact = (repoName, revision) => __async(void 0, null, function* () {
  if (!revision) {
  ...
  if (mainBranch && mainBranch !== branch) {
    ...
    revision = artifactPostfix ? `${revision}-${artifactPostfix}` : `${revision}`;
  ...
  (0, import_fs2.writeFileSync)(workingDir + "/version.conf", `VERSION=${revision}`, {
    encoding: "utf-8"
  });
  yield (0, import_exec3.exec)(`zip ./${buildArtifactFilename} ./build.${packageExtension}`);
  yield (0, import_exec3.exec)(`zip -ur ./${buildArtifactFilename} version.conf`);
